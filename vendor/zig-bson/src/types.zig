...
pub const ObjectId = struct {
    ...
    pub fn makeTimestamp(self: @This()) i32 {
    ...
    pub fn from(allocator: std.mem.Allocator, data: anytype) !Owned(@This()) {
        ...
        owned.value = switch (info) {
            .@"struct" => |v| blk: {
                ...
                break :blk RawBson.makeDocument(fields);
            },
            .optional => blk: {
                if (data) |d| {
            ...
            .pointer => |v| blk: {
                switch (v.size) {
                    ...
                    .slice => {
                        ...
                        var elements = std.array_list.Managed(RawBson).init(owned.arena.allocator());
                        for (data) |elem| {
        ...
        owned.value = switch (@typeInfo(T)) {
                            ...
                            if (doc.get(field.name)) |value| {
                                if (comptime @typeInfo(field.type) == .optional) {
                                    if (value == .null) {
                                        ...
                                        const converted = value.into(owned.arena.allocator(), child_type) catch |err| {
                                            std.debug.print("zig-bson into field {s} as {s} failed with {s}\n", .{
                                                field.name,
                                                @typeName(child_type),
                                                @errorName(err),
                                            });
                                        ...
                                        @field(parsed, field.name) = converted.value;
                                    }
                                } else {
                                    const converted = value.into(owned.arena.allocator(), field.type) catch |err| {
                                        std.debug.print("zig-bson into field {s} as {s} failed with {s}\n", .{
                                            field.name,
                                            @typeName(field.type),
                                            @errorName(err),
            ...
            .array => |v| blk: {
                if (v.child == u8) {
                    switch (self) {
                        .string => |s| {
                            if (s.len != v.len) return error.IncompatibleBsonType;
                ...
                switch (self) {
                    .array => |a| {
                        if (a.len != v.len) return error.IncompatibleBsonType;
                        var result: T = undefined;
                        for (a, 0..) |elem, i| {
                            result[i] = (try elem.into(owned.arena.allocator(), v.child)).value;
                        }
                        break :blk result;
                    },
                    else => return error.IncompatibleBsonType,
                    ...
                    .slice => {
                        ...
                        switch (self) {
                            .array => |a| {
                                ...
                                var elements = try std.array_list.Managed(v.child).initCapacity(owned.arena.allocator(), a.len);
...
test "RawBson.from" {
    ...
    try std.testing.expectEqualDeep(doc.value, RawBson.makeDocument(&.{.{
        "person",
        RawBson.makeDocument(&.{
