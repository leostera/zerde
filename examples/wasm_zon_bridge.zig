...
pub export fn normalize_crew_zon(input_ptr: usize, input_len: usize) bool {
    if (input_ptr == 0) return false;
    const manifest = zerde.wasm.parseFormatWith(zerde.zon, CrewManifest, common.allocator, .{
        .ptr = input_ptr,
        .len = input_len,
    }, .{
        .rename_all = .snake_case,
    ...
    const buffer = zerde.wasm.serializeFormatOwnedWith(zerde.zon, common.allocator, manifest, .{
        .rename_all = .snake_case,
    }, .{}) catch return false;
...
pub export fn crew_bounty_from_zon(input_ptr: usize, input_len: usize) u32 {
